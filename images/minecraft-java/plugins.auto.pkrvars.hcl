minecraft_version = "1.21.4"
plugins = [
  {
    name        = "voicechat"
    url         = "https://hangarcdn.papermc.io/plugins/henkelmax/SimpleVoiceChat/versions/bukkit-2.5.25/PAPER/voicechat-bukkit-2.5.25.jar"
    configfiles = ["voicechat-server.properties"]
  },
  {
    name = "multiverse"
    url = "https://hangarcdn.papermc.io/plugins/Multiverse/Multiverse-Core/versions/4.3.14/PAPER/multiverse-core-4.3.14.jar"
  },
  {
    name= "squaremap"
    url = "https://hangarcdn.papermc.io/plugins/jmp/squaremap/versions/1.3.4/PAPER/squaremap-paper-mc1.21.4-1.3.4.jar"
  },
  {
    name = "noble-whitelist"
    url = "https://hangarcdn.papermc.io/plugins/NobelD/NobleWhitelist/versions/1.2.9/PAPER/NobleWhitelist-1.2.9.jar"
  },
  {
    name = "estools"
    url = "https://github.com/CoPokBl/EsTools/releases/download/5.1.5/EsTools-5.1.5.jar"
  },
  {
    name = "maintenance"
    url = "https://hangarcdn.papermc.io/plugins/kennytv/Maintenance/versions/4.3.0/PAPER/Maintenance-4.3.0.jar"
  },
  {
    name= "plugin-portal"
    url = "https://hangarcdn.papermc.io/plugins/Flyte/PluginPortal/versions/2.2.0/PAPER/PluginPortal-2.2.0.jar"
  }
]
